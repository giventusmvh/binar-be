...
/**
 * Controller for loan application operations. Handles loan submission and tracking for customers.
...
public class LoanApplicationController {
  ...
  /** Submit a new loan application. POST /api/loans Requires LOAN_CREATE permission. */
  @PostMapping
  ...
  public ResponseEntity<ApiResponse<LoanApplicationResponse>> submitLoan(
    ...
    LoanApplicationResponse response =
        loanApplicationService.submitLoan(userDetails.getUsername(), request);
    return ResponseUtil.created("Loan application submitted successfully", response);
  }

  /** Get all loans for current customer. GET /api/loans Requires LOAN_READ permission. */
  @GetMapping
  ...
  public ResponseEntity<ApiResponse<List<LoanApplicationResponse>>> getMyLoans(
    ...
    List<LoanApplicationResponse> response =
        loanApplicationService.getMyLoans(userDetails.getUsername());
  ...
  public ResponseEntity<ApiResponse<LoanApplicationResponse>> getLoanById(
    ...
    LoanApplicationResponse response =
        loanApplicationService.getLoanById(userDetails.getUsername(), id);
    return ResponseUtil.ok(response);
  }
