...
/**
 * Data initializer that seeds the database with initial data on startup. Creates branches, roles,
 * permissions, default users, and products.
 */
@Slf4j
@Component
...
public class DataInitializer implements CommandLineRunner {
  ...
  private void initializeBranches() {
    ...
    Branch jakarta = Branch.builder().code("JKT").location("Jakarta").build();

    Branch surabaya = Branch.builder().code("SBY").location("Surabaya").build();

    Branch bandung = Branch.builder().code("BDG").location("Bandung").build();
  ...
  private void initializePermissions() {
    ...
    List<Permission> permissions =
        Arrays.asList(
            ...
            Permission.builder()
                .code("LOAN_READ_ALL")
                .description("Read all loan applications")
                .build(),
            Permission.builder()
                .code("LOAN_READ_BRANCH")
                .description("Read branch loan applications")
                .build(),
  ...
  private void initializeRoles() {
    ...
    Permission loanApproveMarketing =
        permissionRepository.findByCode("LOAN_APPROVE_MARKETING").orElseThrow();
    Permission loanApproveBranchManager =
        permissionRepository.findByCode("LOAN_APPROVE_BRANCH_MANAGER").orElseThrow();
    Permission loanApproveBackoffice =
        permissionRepository.findByCode("LOAN_APPROVE_BACKOFFICE").orElseThrow();
    ...
    Role superadmin = Role.builder().name(RoleName.SUPERADMIN).permissions(superadminPerms).build();

    // MARKETING - branch-restricted loan processing
    Set<Permission> marketingPerms =
        new HashSet<>(
            Arrays.asList(
                loanReadBranch, loanApproveMarketing, loanReject, productRead, branchRead));
    Role marketing = Role.builder().name(RoleName.MARKETING).permissions(marketingPerms).build();

    // BRANCH_MANAGER - branch-restricted loan approval
    Set<Permission> branchManagerPerms =
        new HashSet<>(
            Arrays.asList(
                loanReadBranch,
                loanApproveBranchManager,
                loanReject,
                productRead,
                branchRead,
                userRead));
    Role branchManager =
        Role.builder().name(RoleName.BRANCH_MANAGER).permissions(branchManagerPerms).build();

    // BACKOFFICE - final approval across all branches
    Set<Permission> backofficePerms =
        new HashSet<>(
            Arrays.asList(loanReadAll, loanApproveBackoffice, loanReject, productRead, branchRead));
    Role backoffice = Role.builder().name(RoleName.BACKOFFICE).permissions(backofficePerms).build();

    // CUSTOMER - loan creation, tracking, profile and plafond management
    Set<Permission> customerPerms =
        new HashSet<>(
            Arrays.asList(
                loanCreate,
                loanRead,
                ...
                plafondRead,
                plafondSelect));
    Role customer = Role.builder().name(RoleName.CUSTOMER).permissions(customerPerms).build();

    roleRepository.saveAll(
        Arrays.asList(superadmin, marketing, branchManager, backoffice, customer));
  ...
  private void initializeUsers() {
    ...
    UserProfile emptyProfile = UserProfile.builder().user(customerEmpty).build();
